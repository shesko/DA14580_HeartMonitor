...
#ifdef __DA14581__
static int PATCHED_581_smpc_pairing_cfm_handler(ke_msg_id_t const msgid,
                                     struct smpc_pairing_cfm *param,
                                     ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Index
...
#endif    
    int p_count = 0; 
		uint8_t  value[7] = {'H','e','l','l','o','\n'};
		uint8_t* init_p = &value[0];
		uint8_t* word_p = init_p;
		int hello_flag = 0;
        ...
        if (stream_queue_more_data( ))
			...
			if(hello_flag>160){
				for(int counter = 0; counter <6; counter ++){
					app_write_ble_buff(word_p);
					if (p_count<5) {p_count++; word_p++;}
					else {p_count = 0; word_p = init_p;}
				}
				hello_flag = 0;
			}
			else hello_flag++;
