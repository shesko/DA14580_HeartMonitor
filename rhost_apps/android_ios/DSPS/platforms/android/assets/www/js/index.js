...
var app = {
	...
	setTextAreaH:function(){
		var inputInfoH=window.innerHeight-document.getElementsByClassName("btn")[2].offsetHeight-document.getElementsByClassName("btn")[3].offsetHeight-document.getElementsByClassName("tab_bar")[0].offsetHeight-document.getElementsByClassName("navigation-bar")[0].offsetHeight-8;
		var inputInfos=document.getElementsByClassName("inputInfo");
		for(var i=0;i<inputInfos.length;i++){
			inputInfos[i].style.height=Math.floor(inputInfoH/2)+"px";
		}
	...
	onDeviceDisconnected: function(arg){
		if(!app.arti_disconnect){
			...
			app.sign_dis_counter++;
			if(app.sign_dis_counter == 2){
				app.sign_dis_counter = 0;
				BC.Bluetooth.StartScan();
			}
			app.deviceOpen();
			setTimeout(function(){
    ...
    gotFS:function(fileSystem){
        ...
        document.getElementById("Directory").addEventListener("touchstart",function(){app.readEntries(fileSystem.root);},false);
    },
    	...
    	entries.sort(function(a,b){
    		...
    		for(var i=0;i<len;i++){
    			if(a.name.charCodeAt(i)-b.name.charCodeAt(i)>0){
    				return 1;
    			}else if(a.name.charCodeAt(i)-b.name.charCodeAt(i)<0){
    				return -1;
    			}else{
    				continue;
    			}
    ...
    readFileAsText : function(fileEntry){
    	fileEntry.file(function(file){
           	...
           	console.log(JSON.stringify(reader.__proto__));
