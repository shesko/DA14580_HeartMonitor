...
- (void)getEnvironment:(CDVInvokedUrlCommand *)command{
    ...
    [info setValue:[pageInfomation valueForKey:@"deviceaddress"] forKey:DEVICE_ADDRESS];
...
- (void)startScan:(CDVInvokedUrlCommand*)command{
    BCLOG_FUNC(GAP_MODUAL)
    NSMutableArray *service_uuids = [[NSMutableArray alloc]
                        initWithArray:[[command.arguments objectAtIndex:0] valueForKey:SERVICE_UUIDS]];
        ...
        if (service.characteristics.count > [characteristicIndex intValue]) {
            ...
            if (descriptorIndex && descriptorIndex.length > 0){
                if (descriptorIndex.intValue < characteristic.descriptors.count) {
                    ...
                    BCLOG_DATA(data,@"writeDescriptor",deviceAddress, descripter.UUID.UUIDString)
                }else{
                    [self error:command.callbackId];
                }
            }else{
                ...
                BCLOG_DATA(data,@"writeCharacteristic",deviceAddress, characteristic.UUID.UUIDString)
            }
        }else{
            [self error:command.callbackId];
        }
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central {
    if ([bluetoothState isEqualToString:BLUETOOTHSTARTSTATE]) {
        ...
        [stateInfo setValue:[pageInfomation valueForKey:@"url"] forKeyPath:@"url"];
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if (!error) {
        ...
        if ([self.callbacks valueForKey:ISON]) {
            ...
            BCLOG_DATA([characteristic value],@"characteristicNotify",deviceAddress, characteristic.UUID.UUIDString)
        ...
        if ([self.callbacks valueForKey:READCHARACTERISTIC]){
            NSMutableDictionary *callbackInfo = [[NSMutableDictionary alloc] init];
            NSString *value = [self encodeBase64:[characteristic value]];
            BCLOG_DATA([characteristic value],@"readCharacteristic",deviceAddress, characteristic.UUID.UUIDString)
            [callbackInfo setValue:value forKey:VALUE];
            [callbackInfo setValue:date forKey:DATE];
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForDescriptor:(CBDescriptor *)descriptor error:(NSError *)error{
    ...
    if (!error) {
        ...
        BCLOG_DATA(aData,@"readDescriptor",[self getPeripheralUUID:peripheral], descriptor.UUID.UUIDString);
...
- (NSMutableDictionary *)storeServiceInfo:(CBPeripheral*)peripheral{
    ...
    for(int i = 0; i < peripheral.services.count; i++){
        ...
        [serviceInformation setValue:[self CBUUIDFiltrToString:service.UUID] forKey:SERVICE_UUID];
...
- (NSString *)CBUUIDFiltrToString:(CBUUID *)UUID{
    ...
    for (int i = 0; i < [[UUID data]length]; i++){
        switch (i){
            ...
            case 5:
            case 7:
            case 9: [string appendFormat:@"%02x-", bytes[i]]; break;
            default:[string appendFormat:@"%02x",  bytes[i]]; break;
        }
    }
    if(string.length == 4){
        string = [[[self expandToTIUUID:[CBUUID UUIDWithString:string]] UUIDString] lowercaseString];
    }
    return string;
}

- (CBUUID *)expandToTIUUID:(CBUUID *)uuid {
    ...
    [uuid.data getBytes:sourceUUIDBytes];
...
- (NSMutableDictionary *)getAdvertisementData:(NSDictionary *)advertisementData
{
    ...
    if ([advertisementData valueForKey:KCBADVDATA_SERVICE_UUIDS]){
        ...
        for (int i = 0; i < advServiceUUIDs.count; i++) {
            [uuids addObject:[[advServiceUUIDs objectAtIndex:i] UUIDString]];
        }
    ...
    if ([advertisementData valueForKey:KCBADVDATA_OVERFLOW_SERVICE_UUIDS]){
        ...
        for (int i = 0; i < overFlowAdvServiceUUIDs.count; i++) {
            [uuids addObject:[[overFlowAdvServiceUUIDs objectAtIndex:i] UUIDString]];
        }
    ...
    if ([advertisementData valueForKey:KCBADCDATA_SOLICITED_SERVICE_UUIDS]){
        ...
        for (int i = 0; i < solicitedAdvServiceUUIDs.count; i++) {
            [uuids addObject:[[solicitedAdvServiceUUIDs objectAtIndex:i] UUIDString]];
        }
